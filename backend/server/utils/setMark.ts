...
export function setMark(
  ...
  const { doc: pNode, mapping } = initProseMirrorDoc(fragment, editor.editor.pmSchema as any);
  ...
  const anchor = relativePositionToAbsolutePosition(doc, fragment, yjsSelection.anchor, mapping);
  const head = relativePositionToAbsolutePosition(doc, fragment, yjsSelection.head, mapping);
  ...
  const tr = setMarkInProsemirror(editor.editor.pmSchema.marks[markName], markAttributes, state);
...
export const setMarkInProsemirror = (type: any, attributes = {}, state: EditorState) => {
  let tr = state.tr;
