  ...
  switch (action.type) {
    ...
    case 'ADD_TASK': {
      ...
      if (targetListIndex !== -1) {
        ...
        if (processSave) {
          draft.listsToUpdate[draft.lists[targetListIndex]._id || draft.lists[targetListIndex].id] =
        ...
        if (processSave) {
          draft.archiveToUpdate[
            draft.archive[targetListIndex]._id || draft.archive[targetListIndex].id
          ] = draft.archive[targetListIndex].tasks;
          eventEmitter.emit('added_material', { projectId: listId, material: newTask });
        }
      }
      break;
    ...
    case 'EDIT_TASK': {
      ...
      if (taskIndex !== -1 && listIndex !== -1) {
        const task =
          location === 'lists'
            ? draft.lists[listIndex].tasks[taskIndex]
            : draft.archive[listIndex].tasks[taskIndex];
        ...
        location === 'lists'
          ? (draft.listsToUpdate[draft.lists[listIndex]._id || draft.lists[listIndex].id] =
              draft.lists[listIndex].tasks)
          : (draft.archiveToUpdate[draft.archive[listIndex]._id || draft.archive[listIndex].id] =
              draft.archive[listIndex].tasks);
      }
      break;
    }
    ...
    case 'REMOVE_LIST': {
      ...
      if (targetListIndex !== -1) {
        if (processSave) {
          draft.listsToRemove[draft.lists[targetListIndex]._id || draft.lists[targetListIndex].id] =
            1;
    ...
    case 'MOVE_TO_ARCHIVE': {
      ...
      if (processSave) {
        draft.archiveToAdd[draft.lists[listIndex]._id || draft.lists[listIndex].id] =
          draft.lists[listIndex];
    ...
    case 'MOVE_FROM_ARCHIVE': {
      ...
      if (processSave) {
        ...
        draft.listsToAdd[draft.archive[listIndex]._id || draft.archive[listIndex].id] =
          draft.archive[listIndex];
    ...
    case 'REMOVE_TASK': {
      ...
      if (targetListIndex > -1) {
        draft.lists[targetListIndex].tasks = draft.lists[targetListIndex].tasks.filter(
          (task) => task.id !== taskId
        );
        if (processSave) {
          draft.listsToUpdate[draft.lists[targetListIndex]._id || draft.lists[targetListIndex].id] =
        ...
        targetListIndex = findItemIndexById(draft.archive, listId);
        if (targetListIndex === -1) return;
        draft.archive[targetListIndex].tasks = draft.archive[targetListIndex].tasks.filter(
          (task) => task.id !== taskId
        );
        if (processSave) {
          draft.archiveToUpdate[
            draft.archive[targetListIndex]._id || draft.archive[targetListIndex].id
          ] = draft.archiveToAdd[targetListIndex].tasks;
          eventEmitter.emit('removed_material', { projectId: listId, material: taskId });
        }
      }
      break;
