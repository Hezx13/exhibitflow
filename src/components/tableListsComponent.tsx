      ...
      <TableRow sx={{ backgroundColor: index % 2 === 0 ? '#00000005' : '#ffffff05' }}>
        <StyledTableCell>
          ...
          <IconButton
            disableRipple
            onClick={() => {
              isArchive ? dispatch(removeList(list.id)) : dispatch(moveToArchive(list.id));
            }}
          >
            {isArchive ? <CloseIcon /> : <ArchiveIcon />}
          </IconButton>
        </StyledTableCell>
        <StyledTableCell>
          <SyledListButton
            onClick={() => {
              handleExpandToTable(list.id);
            }}
          >
            {list.text}
          </SyledListButton>
            ...
            <TableCell>
              {!isArchive && (
                <>
                  ...
                  <IconButton disabled={isProcessing} onClick={onFileUpload}>
