...
const UserManagementTable = ({ onAlert }) => {
  ...
  useEffect(() => {
    ...
    getUserData().then((data) => setActiveUser(data));
  ...
  const handleDeleteUser = async (username: string) => {
    ...
    if (res === 200) {
      fetchUsers().then((data) => {
        if (selectedRow && data.length <= selectedRow) setSelectedRow(data.length - 1);
        setUsers(data);
      });
    }
  };
          ...
          {users.map((row, index) => (
            <TableRow
              ...
              sx={{
                ...
                boxShadow:
                  selectedRow === index
                    ? 'inset -5px 0px 0px 0px orange, inset 5px 0px 0px 0px orange'
                    : '',
                cursor: 'pointer',
                position: 'relative',
            ...
            >
              ...
              <TableCell align="right">
                {(usersInProjects &&
                  lists.find((list) => list.id === usersInProjects[row._id])?.text) ||
                  'Inactive'}
              </TableCell>
              ...
              <TableCell colSpan={5}>
                ...
                >
                  {users[selectedRow].isApproved ? (
                    <Button
                      variant="outlined"
                      onClick={() => handleDisapproveUser(users[selectedRow].username)}
                    >
                      Disapprove
                    </Button>
                  ) : (
                    <Button
                      variant="outlined"
                      onClick={() => handleApproveUser(users[selectedRow].username)}
                    >
                      Approve
                    </Button>
                  )}
                  {users[selectedRow].role === 'Admin' ? (
                    <Button
                      variant="outlined"
                      onClick={() => handleDemoteUser(users[selectedRow].username)}
                    >
                      Demote
                    </Button>
                  ) : (
                    <Button
                      variant="outlined"
                      onClick={() => handlePromoteUser(users[selectedRow].username)}
                    >
                      Promote
                    </Button>
                  )}
                  <Button
                    variant="outlined"
                    onClick={() => handleResetUserPassword(users[selectedRow].username)}
                  >
                    Reset Password
                  </Button>
                  <Button
                    variant="outlined"
                    onClick={() => handleDeleteUser(users[selectedRow].username)}
                  >
                    Delete User
                  </Button>
                </div>
